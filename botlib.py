...
class connection(object):
    def __init__(self, server, port, channels, nick, cb, commands, password="", verbose=False):
    ...
    def go(self):
        ...
        self._send('USER %s 0 0 :bot' % (''.join([chr(randrange(ord('a'), ord('z'))) for i in range(8)])))
        self._send('NICK %s 0' % self.nick)
    ...
    def msg(self, what, msg):
        for line in str(msg).replace('\r', '').split('\n'):
            self._send('PRIVMSG %s :%s' % (what, line))

    def action(self, what, msg):
        self._send('PRIVMSG %s :\001ACTION %s\001' % (what, str(msg)))

    def quit(self, reason="Bot shutting down"):
        self._send('QUIT :' + str(reason))
        self.running = False
    ...
    def names(self, channel):
        ...
        return self._recv().split(':')[2].split()
